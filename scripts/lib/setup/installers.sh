...
function install_leiningen() {
  if is_macos; then
    brew_install leiningen $(getRequiredVersion leiningen) f7e10afc6d04a13d28e825db71326d16c12e9724
  elif is_linux; then
...
function install_leiningen_linux() {
  ...
  local required_version=$(getRequiredVersion leiningen)
...
function install_clojure_cli() {
  if is_macos; then
    local required_version=$(getRequiredVersion clojure_cli)
...
function install_watchman() {
  local required_version=$(getRequiredVersion watchman)
  if is_macos; then
...
function install_maven() {
  local required_version=$(getRequiredVersion maven)
...
function install_react_native_cli() {
  ...
  local required_version=$(getRequiredVersion react_native_cli)
...
function install_yarn() {
  local required_version=$(getRequiredVersion yarn)
...
function install_node_via_nvm() {
  ...
  local required_version=$(getRequiredVersion node)
...
function use_android_sdk() {
  if [ -n "$ANDROID_SDK_ROOT" ]; then
    ...
    local ANDROID_BUILD_TOOLS_VERSION=$(getRequiredVersion ANDROID_BUILD_TOOLS_VERSION)
    local ANDROID_PLATFORM_VERSION=$(getRequiredVersion ANDROID_PLATFORM_VERSION)
...
function install_android_ndk() {
  if grep -Fq "ndk.dir" $_localPropertiesPath; then
    ...
    local ANDROID_NDK_VERSION=$(getRequiredVersion ANDROID_NDK_VERSION)
